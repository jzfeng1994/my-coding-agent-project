...
app.post("/scores", (req, res) => {
  ...
  if (typeof score !== "number" || !Number.isInteger(score) || score < 0 || score > 99999) {
    return res
      .status(400)
      .json({ error: "score must be an integer between 0 and 99999" });
  }

  const trimmedName = name.trim();
  insertScore.run({ name: trimmedName, score });
  res.status(201).json({ name: trimmedName, score });
});

app.listen(PORT, "127.0.0.1", () => {
  console.log(`Phosphor game running at http://127.0.0.1:${PORT}`);
});
